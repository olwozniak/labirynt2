...
public class MazeFileReader {
    ...
    public synchronized static void readTxtToMazeData(File file) throws IOException{
        ...
        if(line == null)
            throw new IOException("Podano pusty plik!");
        ...
        while(line != null)
        {
            ...
            for(int i = 0; i < line.length(); i++)
            {
                switch(line.charAt(i))
                {
                    ...
                    case 'P':
                        if(entry != null)
                            throw new IOException("Znaleziono dwa wejścia w labiryncie!");
                    ...
                    case 'K':
                        if(exit != null)
                            throw new IOException("Znaleziono dwa wyjścia w labiryncie!");
        ...
        if(height < 3 || width < 3)
            throw new IOException("Rozmiar labiryntu jest za mały! (" + width + "x" + height + ")");
